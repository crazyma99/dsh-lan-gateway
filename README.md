...
- **单一密码鉴权边界**：登录成功即拥有完整控制权（含 dsh 设置、凭据配置与插件自身设置），未登录一律 401。网关把 `Host`/`Origin` 重写为 loopback 转发，主服务 `/api` 信任围栏原样生效。
...
**密码鉴权是唯一边界**：能通过登录页/HTTP Basic 鉴权的客户端即拥有与宿主机用户等同的完整控制权（包括修改 dsh 设置、凭据与插件自身配置）。因此请使用强密码，并按需用 IP 白名单收窄来源。完整威胁模型、剩余风险与缓解见 [docs/security.md](docs/security.md)。核心不变量：

- 主服务**从不**绑定非回环地址；
- 未登录请求**一律 401**（浏览器导航走登录页，API/WS 直接拒绝）；
